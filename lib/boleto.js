...
Boleto.prototype._calculate = function (options) {
  var codigo = options['banco'] === 'others' ? options['codigo'] : this.bank.options.codigo

  this['codigo_banco'] = codigo + '-' + formatters.mod11(codigo)
...
Boleto.prototype.renderHTML = function (callback) {
  ...
  if (self['banco'] === 'others') {
    renderOptions.logoURL = self['logoUrl'] || 'https://parcela-sub-api-prod-uploads.s3.us-east-1.amazonaws.com/banco-logos/blank.png'
  }
  ...
  if (self['hide_barcode']) {
    renderOptions['barcode_data'] = '';
  }
